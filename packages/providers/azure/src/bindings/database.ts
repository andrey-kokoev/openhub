...
class AzurePreparedStatement implements PreparedStatement {
  constructor(
  ...
  bind (...values: unknown[]): PreparedStatement {
    return new AzurePreparedStatement(this.transport, this.sql, values)
  }
...
export function createDatabaseBinding (transport: ProxyTransport): DatabaseBinding {
  return {
    prepare (sql: string): PreparedStatement {
      return new AzurePreparedStatement(transport, sql)
    },
    ...
    async batch (statements: PreparedStatement[]): Promise<BatchResult> {
      const rawStatements = statements.map(s => {
        const as = s as AzurePreparedStatement
        // @ts-ignore - accessing private state for transport
        return { sql: as.sql, args: as.args }
      })
