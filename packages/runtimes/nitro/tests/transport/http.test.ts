...
describe('NitroHttpTransport', () => {
  ...
  it('should normalize base URL to include proxy endpoint', () => {
    expect(new NitroHttpTransport('https://example.com', 's').url).toBe(
  ...
  it('should keep URL unchanged when it already ends with proxy endpoint', () => {
    expect(new NitroHttpTransport('https://example.com/__openhub2/proxy', 's').url).toBe(
      'https://example.com/__openhub2/proxy',
    )
  })
  ...
  it('should call fetch with correct headers and body', async () => {
    ...
    const transport = new NitroHttpTransport('https://example.com', 'sekret')
